...
function main()
{
   try
   {
      ...
      if (updateNodeRef !== null)
      {
         ...
         // *********** SMILE **************************/
         // Generacion de registros de auditario para la actualizacion de documentos
     
         var node = search.findNode(updateNodeRef);
         var type = node.getTypeShort();
         var site = node.getSiteShortName();
     	 if(type == "udl:documentSimple") {
		 	audit("url_audit_update_document_simple", updateNodeRef, type, site, null);      
     	 }       
      }
      else {
         /**
         ...
         if (existingFile !== null)
         {
            ...
            if (existingFile.hasAspect("cm:versionable") && overwrite)
            {
               ...
               // *********** SMILE **************************/
               // Generacion de registros de auditario para la actualizacion de documentos
               var nodeRef = existingFile.getNodeRef();
               var node = search.findNode(nodeRef);
               var type = node.getTypeShort();
               var site = node.getSiteShortName();
               if(node.getTypeShort() == "udl:documentSimple") {
	     		 	audit("url_audit_update_document_simple", nodeRef, type, site, null);      
	           }   
         ...
         // *********** SMILE **************************/
         // Generacion de registros de auditario para la creación de documentos
         var nodeRef = newFile.getNodeRef();
         var node = search.findNode(nodeRef);
         var type = node.getTypeShort();
         var site = node.getSiteShortName();
     	 if(type == "udl:documentSimple") {
  		    audit("url_audit_create_document_simple", nodeRef, type, site, null);      
    	 }         
