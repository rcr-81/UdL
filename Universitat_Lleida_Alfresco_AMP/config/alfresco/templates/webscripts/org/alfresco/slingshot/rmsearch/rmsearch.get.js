...
function getRecord(siteId, node)
{
   ...
   for (var k in node.properties)
   {
      ...
      else if (k.match("^{http://www.smile.com/model/udlrm/1.0}") == "{http://www.smile.com/model/udlrm/1.0}")
      {
         item.properties["udlrm_" + k.split('}')[1]] = node.properties[k];
      }
      else if (k.match("^{http://www.smile.com/model/udl/1.0}") == "{http://www.smile.com/model/udl/1.0}")
      {
         item.properties["udlrm_" + k.split('}')[1]] = node.properties[k];
      }
   }
