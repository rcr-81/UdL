...
public class RecalcularTotesDatesAction extends ActionExecuterAbstractBase implements ConstantsUdL{
	...
	protected void executeImpl(Action action, NodeRef nodeRef) {
		System.out.println(DateFormat.getInstance().format(new Date()) + " START: RecalcularTotesDatesAction");
		String usernameAuth = authenticate();
		NodeService nodeService = serviceRegistry.getNodeService();
				
		ResultSet docs = resulgetDocumentsSimples();
		
		int i = 1;
		
		if(docs != null) {
			System.out.println(DateFormat.getInstance().format(new Date()) + " Nº documents: " + docs.length());
			Iterator<NodeRef> iter = docs.getNodeRefs().iterator();
			...
			while (iter.hasNext()) {
				...
				if(nodeService.hasAspect(nodeRef, documentSimple) && !nomDocSimple.equalsIgnoreCase(INDEX)) {			
					System.out.println(DateFormat.getInstance().format(new Date()) + " ########## Document simple " + i + " (" + nodeRef.getId() + ") ##########");
					...
					if(nodeService.hasAspect(expAgrNodeRef, expedient)) {
						updateExpedient(nodeService, nodeRef, expAgrNodeRef);
						
					}else if(nodeService.hasAspect(expAgrNodeRef, agregacio)) {
						updateAgregacio(nodeService, nodeRef, expAgrNodeRef);
					}
					
					updateSerie(nodeService, expAgrNodeRef, serieNodeRef);
					updateFons(nodeService, serieNodeRef, fonsNodeRef);
		...
		System.out.println(DateFormat.getInstance().format(new Date()) + " END: RecalcularTotesDatesAction");
	}
	...
	/**
	 * Busca documentos simples de RM
	 * 
	 * @return
	 */
	private ResultSet resulgetDocumentsSimples() {
		SearchService searchService = serviceRegistry.getSearchService();
		
		SearchParameters sp = new SearchParameters();
		sp.addStore(StoreRef.STORE_REF_WORKSPACE_SPACESSTORE);
		sp.setLanguage(SearchService.LANGUAGE_LUCENE);
		sp.setQuery("ASPECT:\"udlrm:documentSimple\"");
	...
	private void updateAgregacio(NodeService nodeService, NodeRef docNodeRef, NodeRef agrNodeRef) {
		...
		if(agrDataInici == null || "".equals(agrDataInici) || agrDataFi == null || "".equals(agrDataFi)) {
			if(agrDataInici == null || "".equals(agrDataInici)) {
				nodeService.setProperty(agrNodeRef, dataIniExpedientRM, docDataInici);
			}
	
			if(agrDataFi == null || "".equals(agrDataFi)) {
				nodeService.setProperty(agrNodeRef, dataFiExpedientRM, docDataFi);
			}
	...
	public void runRecalcularTotesDates(ServiceRegistry serviceRegistry){
