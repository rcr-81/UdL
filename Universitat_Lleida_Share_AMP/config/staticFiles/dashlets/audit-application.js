				...
				formatter:function(elCell, oRecord, oColumn, oData) 
				    {
						...
						{
							var link_start="";
							if(audit_value.indexOf("cm:folder")>=0 | audit_value.indexOf("udl:expedient")>=0 | audit_value.indexOf("udl:agregacio")>=0 
									| audit_value.indexOf("udl:serie")>=0 | audit_value.indexOf("udl:expedientFisic")>=0 | audit_value.indexOf("udl:fons")>=0 
									| audit_value.indexOf("udl:grupDeFons")>=0){
								  link_start="<a href='"+Alfresco.constants.URL_PAGECONTEXT+"folder-details?nodeRef=";
							}else{
								  link_start="<a href='"+Alfresco.constants.URL_PAGECONTEXT+"document-details?nodeRef=";
							}
						...
						arrayAsString += (multivalued ?"[ ":"") + (audit_key+"").swapHighlightMarkers()
								     + "=" + displayed_value + (multivalued ?" ] ":"");
					    }
					    arrayAsString = arrayAsString.replace("UserName","Usuari");
					    arrayAsString = arrayAsString.replace("Action","Acció");
					    arrayAsString = arrayAsString.replace("DocumentType","Tipus");
					    arrayAsString = arrayAsString.replace("Site","Lloc");
					    arrayAsString = arrayAsString.replace("NodeRef","Node"); 
