...
(function RM_Audit()
{
   ...
   YAHOO.extend(Alfresco.RM_Audit, Alfresco.component.Base,
   {
         ...
         if (this.options.viewMode==Alfresco.RM_Audit.VIEW_MODE_DEFAULT)
         {
            ...
            DS.doBeforeCallback = function ( oRequest , oFullResponse , oParsedResponse , oCallback )
            {
               ...
               if(oCallback.argument == '') {
               		...
               		maxNumEntries = oCallback.argument;
               }
               ...
               //sml - Filter by event (minag)
               var data = oFullResponse.data.entries || []; 
               var filtered = []; 
               var i,l; 

			   if(oRequest) {               	 
	               for (i = 0, l = data.length; i < l; ++i) { 
	            	   if (data[i].event == "Consulta Document") { 
	            	   	   if(filtered.length < maxNumEntries) {
      ...
      _buildQuery : function RM_Audit__buildQuery()
      {
         ...
         qs = qs.join('&');
         return '?event=Consulta%20Document&' + qs;
      },
      ...
      onApplyFilters : function RM_Audit__applyFilters()
      {
         ...
         for (var i=0,len = filterMap.length;i<len;i++)
         {
            ...
            if (uiValue!=="")
            {
               if(mapping.queryId == "size") {
           			this.queryParams[mapping.queryId] = 9999999;               		
      ...
      _query : function RM_Audit__reQuery()
      {
         ...
         var oCallback = {
            ...
            argument : Dom.get(this.id+'-entries').value
         };
